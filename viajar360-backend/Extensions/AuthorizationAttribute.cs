...
namespace Viajar360Api.Extensions
{
    public class AuthorizationAttribute : TypeFilterAttribute
    {
        ...
        public class ClaimRequirementFilter : IAsyncActionFilter
        {
            ...
            public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
            {
                ...
                // just one role for user in this viajar360 project
                var rolesString = context.HttpContext.User.Claims.FirstOrDefault(x => x.Type == "Rol")?.ToString().Remove(0, 7);
                if (rolesString != null)
                {
